...
export function AdminDashboard() {
  ...
  function exportParticipants() {
    const rows = filteredParticipants.map((p) => ({
      ...
      currentTop: formatTopScores(p.current_scores),
      desiredTop: formatTopScores(p.desired_scores),
  ...
  const currentTop = data.aggregate.responseCount > 0 ? topArchetypes(data.aggregate.currentPct) : [];
  const desiredTop = data.aggregate.responseCount > 0 ? topArchetypes(data.aggregate.desiredPct) : [];
  const currentTopValue = currentTop[0] ? data.aggregate.currentPct[currentTop[0]] : null;
  const desiredTopValue = desiredTop[0] ? data.aggregate.desiredPct[desiredTop[0]] : null;
    ...
    <div className="space-y-6">
      ...
      {tab === "overview" && (
        <div className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            <CultureTopCard
              title="Current Culture เด่น"
              subtitle="รูปแบบการทำงานที่เกิดขึ้นจริงและถูกเลือกมากที่สุด"
              archetypes={currentTop}
              value={currentTopValue}
              tone="blue"
            />
            <CultureTopCard
              title="Desired Culture เด่น"
              subtitle="รูปแบบการทำงานที่พนักงานอยากเห็นและถูกเลือกมากที่สุด"
              archetypes={desiredTop}
              value={desiredTopValue}
              tone="emerald"
            />
          </div>
        ...
        <div className="space-y-5">
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto_auto]">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="ค้นหารหัส ชื่อ BU หรือฝ่าย"
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-3 font-semibold"
              >
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Export คำตอบทั้งหมดเพื่อวิเคราะห์ต่อ หรือ Sync ข้อมูลล่าสุดจาก Supabase ไปยัง Google Sheets กลางของ HR
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={exportAllResponses}
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 font-semibold hover:bg-slate-50"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white disabled:opacity-50"
                >
              ...
              <table className="w-full min-w-[1180px] text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="p-3 text-left">Current เด่น</th>
                    <th className="p-3 text-left">Desired เด่น</th>
                  ...
                  {filteredParticipants.map((participant) => (
                    <tr key={participant.employee_id} className="border-t border-slate-100">
                      ...
                      <td className="p-3 text-center">
                        <StatusBadge complete={participant.completed} />
                      </td>
                      <td className="p-3">
                        <TopArchetypeBadges scores={participant.current_scores} tone="blue" />
                      </td>
                      <td className="p-3">
                        <TopArchetypeBadges scores={participant.desired_scores} tone="emerald" />
                      </td>
                      <td className="p-3">{participant.submitted_at ? formatDate(participant.submitted_at) : "-"}</td>
                      <td className="p-3 text-center">
                        <button
                          disabled={!participant.answers}
                          onClick={() => setSelected(participant)}
                          className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 font-medium disabled:cursor-not-allowed disabled:opacity-35"
                        >
                          <Eye size={16} /> ดูคำตอบ
                        </button>
                      </td>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="card p-5 text-sm leading-6 text-slate-600">
              ...
              <p className="mt-3">
                ข้อมูลส่วนนี้เปิดให้เฉพาะ Admin เพื่อใช้ตรวจสอบความครบถ้วน วิเคราะห์ Culture และประกอบการวางแผนภายใน เช่น Engagement,
                Talent หรือ Succession โดยควรใช้เป็นข้อมูลประกอบ ไม่ใช้คำตอบชุดเดียวตัดสินบุคคลโดยลำพัง
              </p>
            </div>
          </div>
        </div>
      ...
      {selected && (
        <IndividualResponseModal participant={selected} surveyVersion={data.surveyVersion} onClose={() => setSelected(null)} />
      )}
    </div>
  );
}

function CultureTopCard({
  title,
  subtitle,
  archetypes,
  value,
  ...
  return (
    <div className={`rounded-2xl border p-5 ${toneClass}`}>
      ...
      <div className="mt-1 text-xs leading-5 opacity-70">{subtitle}</div>
      {archetypes.length ? (
        <div className="mt-4 flex flex-wrap items-end justify-between gap-3">
          <div className="flex flex-wrap gap-2">
            {archetypes.map((key) => (
              <span key={key} className={`rounded-full px-3 py-1.5 text-sm font-bold ${pillClass}`}>
        ...
        <div className="mt-4 text-sm font-semibold opacity-70">ยังไม่มีคำตอบ</div>
      )}
    </div>
  );
}
          ...
          <div key={key} className="card p-5">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="text-2xl font-bold">{aggregate.currentPct[key]}%</div>
              </div>
              <div>
                <div className="text-xs text-slate-400">Desired</div>
                <div className="text-2xl font-bold">{aggregate.desiredPct[key]}%</div>
              </div>
            </div>
            <div
              className={`mt-3 text-sm font-semibold ${
                aggregate.gapPct[key] > 0
                  ? "text-emerald-700"
                  : aggregate.gapPct[key] < 0
                    ? "text-rose-700"
                    : "text-slate-500"
            ...
            >
              Gap {aggregate.gapPct[key] > 0 ? "+" : ""}
              {aggregate.gapPct[key]} จุด
            </div>
          </div>
        ))}
      </div>
...
function IndividualResponseModal({
  participant,
  surveyVersion,
  onClose,
}: {
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5 sm:p-6">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold">
              {[participant.name, participant.surname].filter(Boolean).join(" ") || participant.employee_id}
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              {participant.employee_id} · {[participant.bu, participant.department, participant.job_level].filter(Boolean).join(" / ")}
            </p>
          </div>
              ...
              {ARCHETYPES.map((key) => (
                <div key={key} className="rounded-2xl bg-slate-50 p-4">
                  ...
                  <div className="mt-2 text-sm text-slate-600">
                    Current {participant.current_scores?.[key] ?? 0} / Desired {participant.desired_scores?.[key] ?? 0}
                  </div>
                </div>
              ))}
            </div>
...
function AnswerBox({
  ...
  return (
    <div className={`rounded-xl border p-4 ${classes}`}>
      ...
      <div className="mt-1 text-sm leading-6">{label ?? "-"}</div>
      <div className="mt-2 text-xs font-semibold opacity-70">{archetype ? ARCHETYPE_META[archetype].label : "-"}</div>
    </div>
  );
}

function TopArchetypeBadges({ scores, tone }: { scores: Scores | null; tone: "blue" | "emerald" }) {
  const top = topArchetypes(scores);
  if (!top.length) return <span className="text-slate-400">-</span>;
  const classes = tone === "blue" ? "bg-blue-50 text-blue-700" : "bg-emerald-50 text-emerald-700";
  return (
    <div className="flex flex-wrap gap-1.5">
      {top.map((key) => (
        <span key={key} className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${classes}`}>
          {ARCHETYPE_META[key].label}
        </span>
      ))}
    </div>
...
function topArchetypes(scores: Scores | null) {
  if (!scores) return [] as Archetype[];
  const values = ARCHETYPES.map((key) => Number(scores[key] ?? 0));
  const total = values.reduce((sum, value) => sum + value, 0);
  if (total <= 0) return [] as Archetype[];
  const max = Math.max(...values);
  return ARCHETYPES.filter((key) => Number(scores[key] ?? 0) === max);
}

function formatTopScores(scores: Scores | null) {
  const top = topArchetypes(scores);
  return top.length ? top.map((key) => ARCHETYPE_META[key].label).join(" + ") : "";
}
